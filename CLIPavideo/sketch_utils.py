...
def save_sketches_video(sketches, output_dir, title):
    ...
    writer_edit = imageio.get_writer(
        f"{output_dir}/{title}", fps=10)
    # writer_edit = imageio.get_writer(
    #     f"{output_dir}/{title}", fps=2)
    sketches_video = np.zeros((224, 224, 3, len(sketches)))
    for idx, item in enumerate(sketches):
        grid = make_grid(item, normalize=False, pad_value=2)
        npgrid = grid.detach().cpu().numpy()
        sketches_video[:,:,:,idx] = np.transpose(npgrid, (1, 2, 0))

    for i in range(len(sketches)):
        writer_edit.append_data((sketches_video[:, :, :, i] * (255)).astype(np.uint8))
